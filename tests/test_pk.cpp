...
size_t plaintext_length = 14;
std::uint8_t *plaintext = (std::uint8_t *) "This is a test";
...
std::cout << "encrypt #1 ["<< result <<"]" << std::endl;
for (std::uint8_t c : output_buffer) {
  std::cout << c;
}
std::cout << std::endl;
...
// plaintext = (std::uint8_t *)"asdasd";
// std::cout << "decrypt #1:" << std::endl
//           << plaintext_buffer << std::endl;

result = olm_pk_decrypt(
    decryption,
    ephemeral_key.data(), ephemeral_key.size(),
    output_buffer.data(), output_buffer.size(),
    ciphertext_buffer, ciphertext_length,
...
std::cout << "decrypt #2 [" << result << "]" << std::endl
          << plaintext_buffer << std::endl;
...
size_t res = olm_pk_signing_key_from_seed(
    signing,
    pubkey.data(), pubkey.size() - 1,
    seed, sizeof(seed)
);

std::cout << "--here " << res << "/" << pubkey.data() << std::endl;
